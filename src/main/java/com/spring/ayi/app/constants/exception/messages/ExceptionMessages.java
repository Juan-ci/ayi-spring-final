package com.spring.ayi.app.constants.exception.messages;

public class ExceptionMessages {

    public static final String DOCUMENT_ALREADY_EXIST = "DOCUMENT NUMBER {0} ALREADY EXISTS.";

    public static final String MAIL_ALREADY_EXIST = "MAIL {0} ALREADY EXISTS.";

    public static final String ADDRESS_ID_NOT_FOUND = "ADDRESS ID {0} NOT FOUND.";

    public static final String CLIENT_ID_NOT_FOUND = "CLIENT ID {0} NOT FOUND.";

    public static final String USER_ACCOUNT_ID_NOT_FOUND = "USER ACCOUNT ID {0} NOT FOUND.";

    public static final String USER_ACCOUNT_MAIL_NOT_FOUND = "USER ACCOUNT MAIL {0} NOT FOUND.";

    public static final String DOCUMENT_NUMBER_NOT_FOUND = "DOCUMENT NUMBER {0} NOT FOUND.";

    public static final String EMPTY_LIST_EXCEPTION = "LIST OF {0} IS EMPTY.";

    public static final String PAGE_DOES_NOT_EXIST = "PAGE {0} WITH {1} REGISTER PER PAGE DOES NOT EXIST.";
}
